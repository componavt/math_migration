...
def detest(testkey):#Description of problem
      import numpy as np
      ivpoptim=IVPOPTIM()
      if testkey=='2ODu':# differential equation
...
plt.axis([0, 5,0,10])
plt.plot(t, z,"r-")#model solution
plt.xlabel('t')
plt.ylabel('u')
plt.legend(['z(t),a0=-1'], loc = 'central')
plt.grid()
plt.show()
def detest(testkey):#Description of problem
      import numpy as np
      ivpoptim=IVPOPTIM()
      if testkey=='2OD':#differential equation with parameter a0 which is not (-1)      
        ivpoptim.u0=u0#initial value
        ivpoptim.T=3.0#Final time of integration
        ivpoptim.rhs =lambda t,u:a0*u  #right side of ODE 
        ivpoptim.dt0 = 0.01#time step
       
      elif testkey=='2OD1':# differential equation with parameter changed by an amount dq
        ivpoptim.u0=u0#initial value
        ivpoptim.T=3.0#Final time of integration
      ...
      elif testkey=='2OD11':#differential equation with parameter changed by an amount 2dq
         ivpoptim.u0=u0#initial value
         ivpoptim.T=3.0#Final time of integration
...
while(norma>=eps1):  
   ...
   #Construct the Hessian matrix:
    ...
    if (funclist[i-1]> funclist[i]):
                ...
                plt.axis([0, 5,0,10])
                plt.plot(t, z,"r-")#model solution
                plt.plot(t, u,"g-" )#model solution
                plt.xlabel('t')
                plt.ylabel('u')
                plt.legend(['z(t),a0=-1','u(t)'], loc = 'central')
                #plt.legend(['x(t)'], loc = 'central')
                plt.grid()
                plt.show()
    if (funclist[i-1]<=funclist[i]):
              ...
              plt.ylabel('u')
              plt.legend(['z(t),a0=-1','u(t)'], loc = 'central')
              #plt.legend(['u(t),a0=-1'], loc = 'central')
              #plt.legend(['x(t)'], loc = 'central')
              plt.grid()
              plt.show()
