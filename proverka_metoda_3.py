...
print(norma)
fig=[plt.plot(t, z,"r-",lw=0.5)]

alist=[]#empty list of parameters a0

while(norma>=eps1):
    alist.append(a0)#list of parameters a0
   ...
   #plot of model solution in i step
    print('number of iteration:')
    print(i)
...
for a0 in alist:#solution of ODE1 on i step with new parameters a0
     ...
     plt.plot(t, u,"k-",lw=(j+1)/j)
     fig.append(plt.plot(t, u,"k-",lw=(j+1)/j)) 
#plots    
plt.axis([0, 3,0,10])
plt.plot(t, z,"r-",lw=0.5)#plot of "ecact" solution
plt.xlabel('t')
plt.ylabel('u')
plt.legend(['z(t),a0=-1','u(t)'], loc = 'center')              
plt.grid(fig)
plt.show(fig)
