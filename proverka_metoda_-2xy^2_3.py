...
#if (norma<=eps1):
        #a0=a0  
alist=[]#empty list of parameteres a0
...
while(norma>=eps1):
    ...
    u3=np.array([m[1667], m[3334], m[5001]])        
...
for a0 in alist:#solution of ODE1 on i step with new parameters a0
     ...
     plt.plot(t, u,"k-",lw=0.1)
     fig.append(plt.plot(t, u,"k-",lw=0.1)) 
    
    
#plots    
plt.axis([0,3,0,1])
plt.plot(t, z,"r--",lw=0.5)#plot of "ecact" solution

plt.xlabel('t')
plt.ylabel('u')
plt.legend(['z(t),a0=-2','u(t)'], loc = 'center')              
plt.grid(fig)
plt.show(fig)
