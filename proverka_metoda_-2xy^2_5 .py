...
a0=-1. #initial value of parameter

u0=1.#initial solution in moment time 0 
dq=0.00001
funclist=[ ]#empty list of function value, which must be minimized
...
def detest(testkey):#Description of problem
      ...
      if testkey=='Differential equation for exact solution':#differential equation
                         #with parameter which is (-2), i.e. we think, that a0=-2,
...
def detest(testkey):#Description of problem
      import numpy as np
      ivpoptim=IVPOPTIM()
      if testkey=='Differential equation':#differential equation with parameter a0 which is not (-2)      
        ...
        ivpoptim.u0=u0#initial value
        ivpoptim.T=3.0#Final time of integration
        ivpoptim.rhs =lambda t,u: (a0+dq)*t*(u**2)
        ivpoptim.dt0 = 0.01
         ...
         ivpoptim.u0=u0#initial value
         ivpoptim.T=3.0#Final time of integration
         ivpoptim.rhs = lambda t,u: (a0+2*dq)*t*(u**2)
         ivpoptim.dt0 = 0.01     
...
print(norma)
alist=[a0]#list of parameteres a0
...
while(norma>=eps1):
    ...
    t,m = rk44(myivp3)#approximate soluton of problem    

    u3=np.array([m[r1], m[r2], m[r3]])        
...
#plot for solution
plt.axis([0, 3,0,1])
plt.plot(t, z,"r-",lw=0.8)#exact solution
...
for a0 in alist:#solution of ODE1 on i step with new parameters a0
     ...
     plt.plot(t, u,"g--",lw=0.5)#approximate solution
     fig.append(plt.plot(t, u,"g--",lw=0.5)) 
...
plt.plot(t, z,"r-",lw=0.8)
plt.xlabel('t')
plt.ylabel('u')
plt.legend(['z(t),dz/dt = -2*t*z^2','u(t),du/dt = -a0*t*u^2'], loc = 'right up')              
plt.grid(fig)
plt.show(fig)
...
for i in ilist:
   plt.plot(i, totalpogrlist[i],"ko",lw=0.07)
   fig1.append(plt.plot(i, totalpogrlist[i],"ko",lw=0.07))
